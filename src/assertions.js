...
export default original => {
  const toHaveProp = addEnzymeSupport(original.toHaveProp, function(
    wrapper,
    prop,
    value,
  ) {
  ...
  return {
     ...
     */
    toHaveProps: addEnzymeSupport(original.toHaveProps, function(
      actual,
      props,
    ) {
      ...
      const key = Object.keys(props).find(key => {
        const value = props[key];

        // Assert!
        const assertion = toHaveProp.call(this, actual, key, value);
        return assertion.pass === this.isNot;
      });

      if (key) return toHaveProp.call(this, actual, key, props[key]);
     ...
     * @return {this} - The expectation context.
     */
    toHaveRendered: addEnzymeSupport(original.toHaveRendered, function(
      element,
    ) {
      const { actual } = this;
